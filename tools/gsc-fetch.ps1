...
# Usage: .\gsc-fetch.ps1 ie    (or: uk | es | pt | _analysis_ie | all)
...
$map=@{ ie="ie_tatkowski.com"; uk="uk_tatkowski.co.uk"; es="es_tatkowski.es"; pt="pt_tatkowski.pt"; analysis="_analysis_ie"; "_analysis_ie"="_analysis_ie" }
...
foreach($folder in $folders){
  ...
  foreach($name in ($list -split "`n" | Where-Object { $_ })){
    $b64 = (Gh @("api","repos/$Repo/contents/gsc/$folder/$name?ref=$Branch","--jq",".content")) -join "" -replace "\s",""
