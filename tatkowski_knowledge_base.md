...
- PT address (for citations/NAP): Avenida São João de Deus, Edifício Príncipe Real, Lote 1, 1C, 8500-508 Portimão, Algarve, Algarve
...
NOTE (fixed 08/06/26): pull.js had a second bug — it marked the CURRENT in-flight month as "done" in _progress.jsonl after the first pull, so subsequent runs skipped it and silently returned no new data even though days kept accumulating. Fix: pull.js now classifies each month as ACTIVE (current month always; previous month only on days 1-3 of current to cover GSC's 2-3 day reporting lag) vs FROZEN (everything older). Active months are re-pulled every run and never written to _progress.jsonl. Frozen months are pulled once then skipped permanently. Manual `npm run pull` uses the same logic. Log is now truthful: shows Total combos / Frozen (skipped) / To fetch this run (active + new frozen). Verified 08/06/26 — Sunday 07/06 run had silently no-op'd; manual rerun pulled 20 active combos, sync committed 20 files to KB (4f8130019).
...
Source code lives in this repo at `gsc-tools/` (committed 08/06/26, commit 55019b1). Executing copy stays at `D:\tatkowski-gsc\` on Maciej's machine where node_modules, data/, logs/, .env remain local. When puller code changes: edit working copy → mirror to gsc-tools/ → commit.
...
| Tool | Purpose | Notes |
|---|---|---|
| Microsoft 365 (contact@tatkowski.com) | Email, calendar, Planner, SharePoint, Teams | M365 connector active. Always use mailboxOwnerEmail: contact@tatkowski.com. Calendar SEARCH only via MCP — no create. Auto-renewed 3 June 2026. **M365 Group "Tatkowski Interpreting & Recruitment"** (renamed 06/06/26 from "Interpretation", typo fix) backs SharePoint, Planner, Teams, group mailbox. Primary: team@tatkowski.com (fallback alias: team@tatkowski.onmicrosoft.com). Owners: Maciej, Magdalena. Members: Maciej, Magdalena, David, Artur. **contact@ explicitly NOT a member** — keeps client-facing mailbox clean of internal noise. team@ is internal coordination only, never client-facing. Planner is the task delegation surface (Outlook Tasks pane = personal only, no assignment). First plan TBD. |
...
| **Domain architecture — RESOLVED 10/06/26: STATUS QUO CORRECT** | Maciej | Raised 10/06/26 post-GBP-PT-verification, resolved same day. Initial framing (Claude) suggested consolidation under one domain due to "backlink dilution" — that framing was WRONG. Each ccTLD has its own geo-locked backlink portfolio (PT directories will only link to .pt, UK to .co.uk, etc.) so the portfolios are PARALLEL not COMPETING. Current 4-ccTLD setup is the correct architecture for geo-targeted B2C translation across IE/UK/ES/PT — already producing working per-market ranking signals (ccTLD + local NAP + local citations + dedicated GBP). Consolidation cost (re-pointing 100+ citations, changing GBP website URLs which would re-trigger review on all 3 verified profiles, risk of ranking loss during 301 transition, monorepo rewiring) outweighs any future brand-backlink gain we don't yet have. CONSEQUENCE: keep .com, .co.uk, .pt, .es as independent geo-targeted sites. |
| **Country switcher across all 4 sites** | Maciej | NEW 10/06/26 — derived from domain architecture decision. Scope: (1) Add country picker to global nav/menu on all 4 sites (IE/UK/ES/PT). Visible flags or "IE / UK / ES / PT" buttons. Hover/tap → switch to corresponding ccTLD. (2) Add country-selector module to home/landing page hero on .com — primary entry point for cross-market traffic. (3) Optional: geo-IP soft suggestion banner ("It looks like you're in Portugal — visit our PT site?") with dismissible cookie. Hard redirect would break direct .com links; suggestion only. Strategic value beyond UX: visible multi-country presence in nav = credibility signal for B2B prospects (Fyffes-class clients want suppliers who look international). Implementation lives in packages/ui (shared component), wired into each app's layout. Roadmap file: scope as roadmap/country-switcher.md before code work. |
...
| **SayMore wiring fault — INCIDENT** | Maciej | DISCOVERED 11/06/26. SayMore subscription IE_38029628_635116_15279 keyed to **Pro Rubbish Removal's Place ID** (Strand Street Malahide, prorubbishremoval.ie, 087 294 2555, 82 reviews) — not ours. Two of their reviews pre-date our incorporation (Dec 2025) so this cannot be a data merge; it's wrong Place ID from setup. The AI Review Responses tool has been **auto-posting owner replies on Pro Rubbish Removal's actual Google profile** under our subscription since first response 29/11/25. CONFIRMED LIVE: response text on their public GBP matches SayMore "Live" status verbatim ("Thank you for the feedback from pro rubbish removal"). ~7 months of impersonation-shaped responses on another business's GBP traceable to subscriber 38029628. Action 12/06: call Enoma, 3 demands — (1) stop AI tool immediately, (2) written confirmation of how our subscription was authenticated to post on another business's GBP, (3) repoint subscription to actual Tatkowski Place ID. Don't touch SayMore — preserve state as evidence. Recap call by email after for paper trail. Screenshots filed (11/06 chat). Strengthens FCR cancel position for 31/10/26 window. |
| Enoma follow-ups (GBP/SayMore reset) | Maciej | Email sent 11/06/26: GBP verified close-out + 5 asks. Second email 11/06 with description copy + first post draft. Phone calls 11/06: (1) Enoma claimed first email didn't arrive (sent from maciej@; resent forward from contact@ landed); (2) follow-up call — he had it but was "trying to understand" 5 simple bullets. Pattern: drifty on soft asks, responsive only to transactional ones. Chase if no reply by 18/06/26. Note for Oct cancel decision: friction documented. |
