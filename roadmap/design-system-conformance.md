...
- **11/06/26 — H1 fix on 6 UK/ES/PT base-flagship pages = deliberate improvement, shipped in Phase B (not deferred).** Issue #028: UK/ES/PT polish-translation + ukrainian-translation render with zero `<h1>` tag (cloned bug). Phase B `LandingPage` template emits an H1 by default; migration fixes the bug as a side effect. SEO gate normally enforces byte-faithful (h1 exact match) — for these 6 pages the gate must allowlist a deliberate `h1: 0 → 1` delta with explicit text noted. Cost of fix during migration = zero. All other gates (title, meta, canonical, h2 set, internalLinks, JSON-LD semantic, wordCount ≥95%) must still pass. Decided by Maciej. Side-note: IE polish-translation H1 contains per-page pricing ("€39.99/Page") which contradicts the "quote totals only" rule — out of Phase B template scope, flagged for a separate IE-flagship content review.
