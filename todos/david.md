# To-Do - David Briceag (Regional Manager, Spain & Portugal)
_Source of truth for David's tasks. Key items mirrored to KB. Updated 05/06/26._

## Now
- [ ] Sign Contractor Agreement + non-compete (drafted May 2026, still unsigned - operational risk).
- [ ] Confirm 931 052 617 has NEVER been registered on the WhatsApp / WhatsApp Business app (needed for AI Cloud API - must be a fresh number).
- [ ] Send GitHub username so Maciej can add you as repo collaborator.

## Next
- [ ] ES AI channel: source 1 physical PAYG SIM (PT AI = 931 052 617 already sorted).
- [ ] PT/ES localised service pages (with Maciej) - currently redirected to English.

## Done (recent)
- [x] 05/06 - PT BrightLocal campaign live (972979) on your company WhatsApp 931 052 612.
- [x] 04/06 - Provided PT NAP address (Portimao) + company Business WhatsApp 931 052 612.
