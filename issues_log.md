...
- Resolution: RESOLVED-INVALID 11/06/26 by Claude (Code, Opus 4.7) during Phase B sentinel migration. The symptom was a measurement artefact in the original cross-market recon catalogue — these 6 pages DO carry an `<h1>`, emitted via `LangHero` (`packages/ui/src/components/LangHero.astro:90`: `<h1 class="hero-title">{h1}<br><span class="lh-accent">{accent}</span></h1>`). Pre-Phase-B snapshots via `tools/seo-snapshot-market.mjs` confirm: `pre-uk/polish-translation.seo.json` extracts `"h1": "Polish Translation London & UK"`. The "h1: 0" recon figure likely came from a counting method that missed LangHero's `class="hero-title"` h1. No deliberate-improvement allowlist needed; Phase B sentinels migrated under strict byte-faithful h1 equality. Phase B journal `docs/phase-b-progress.md` § "Material finding" details the discovery. Phase B commit: 9e9b21b.
- Recurrence: 1
